"""Train a CNN to estimate Rocky Creek Branch water level from webcam images.

INSTALL (once):  pip install torch torchvision pandas pillow numpy

DATA YOU MUST IMPORT
--------------------
1. Input images: put the original Rocky Creek images in IMAGE_DIR.  Every image
   name must start with its capture time in this format: YYYYMMDD_HHMMSS.jpg
   Example: 20260826_123000.jpg
2. USGS target data: export a CSV to USGS_CSV with columns named `dateTime`
   and `gage_height_ft`. Timestamp values should be ISO dates, for example
   2026-08-26T12:30:00Z. Change the column names below if yours differ.
The model uses two convolution layers, two max-pooling layers, ReLU, a linear
output, Huber loss, 80/20 train-validation split, batch size 8, up to 100
epochs, and validation-loss early stopping.
"""
...
# Replace these two paths with the folders/files on your computer.
